...
import { formatBytes, parentFolderName } from "@/lib/desktop-utils";
import { useDesktopStore } from "@/stores/desktopStore";
import type { ActionReport } from "@/types/desktop";
 ...
 * selected item even when an earlier one fails, so a report can say "23 moved,
 * 2 could not be" — and the two are named, with the reason, because an item
 * the user believes was dealt with and was not is the worst outcome this
 * feature can produce.
 *
 * The reasons are worth reading here more than in Downloads: a desktop
 * failure is usually a refusal rather than a fault — the public desktop, a
 * folder, something that was already gone — and the sentence Rust sends says
 * which.
 *
 * A move also names where each item went. The destination filename can differ
 ...
 */
function DesktopActionReport({ report }: { report: ActionReport }) {
  const dismissReport = useDesktopStore((state) => state.dismissReport);
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <p className="flex items-center gap-2 text-sm">
          ...
          <span>
            {report.succeeded > 0
              ? `${report.succeeded} ${report.succeeded === 1 ? "item" : "items"} ${isMove ? "moved" : "deleted"} · ${formatBytes(report.bytes)}`
              : "Nothing was changed"}
            {failures.length > 0 &&
...
export default DesktopActionReport;
